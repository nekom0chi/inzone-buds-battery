...
- 白モデルのイヤホン画像で、円形の音導入口を実機に合わせた黒いメッシュへ修正しました。

### v1.2.0

- 専用画面をナイトモードへ変更しました。
- 黒・白・グラスパープルの製品画像を右上の色見本から選べるようにしました。
- Rはラベルだけを赤くし、L/Rのパーセントは同じ白色で表示します。
- WEEKとDAYの縦棒グラフを上下へ同時表示します。
- WEEKの曜日をクリックすると、その日（00:00～24:00）のDAYグラフへ切り替わります。
- EQ欄から6種類のプリセットを選び、INZONE HUBを開かずにGame出力のEQへ反映できるようにしました。

### v1.1.2

- 専用画面を440 x 480ピクセルへ小型化しました。
- 通知領域をクリックしたモニターの右下端へ固定表示します。
- INZONE HUBのログから現在のEQプリセットを表示します。
- EQ表示をクリックするとINZONE HUBが開き、公式画面からEQを変更できます。
...
- `INZONE-Buds-Battery-Setup-v1.2.1.exe`
...
```text
dist-native\INZONE-Buds-Battery-Setup-v1.2.1.exe
```
