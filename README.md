...
右上のEQ表示には、INZONE HUBのログから読み取った現在のプリセット名が表示されます。クリックするとINZONE HUBを開きます。INZONE HUBには外部アプリ向けの公開APIがないため、このツールから設定ファイルを直接書き換えることはせず、EQの変更とイヤホンへの反映は公式画面で行います。
...
- `INZONE-Buds-Battery-Setup-v1.1.2.exe`
...
```text
dist-native\INZONE-Buds-Battery-Setup-v1.1.2.exe
```
